...
@Singleton
internal class Upgrade15To16 @Inject constructor() : DatabaseUpgrade(15, 16) {

	override fun internalApplyTo(db: Database, origin: Int) {
		db.beginTransaction()
		try {
			addDescriptionToVaultSchema(db)
	...
	private fun addDescriptionToVaultSchema(db: Database) {
		// Step 1: Rename old table
		Sql.alterTable("VAULT_ENTITY").renameTo("VAULT_ENTITY_OLD").executeOn(db)

		// Step 2: Create new table with DESCRIPTION column
		...
		Sql.insertInto("VAULT_ENTITY") //
			.select("_id", "FOLDER_CLOUD_ID", "FOLDER_PATH", "FOLDER_NAME", "FORMAT", "CLOUD_ENTITY.TYPE", "PASSWORD", "PASSWORD_CRYPTO_MODE", "POSITION", "SHORTENING_THRESHOLD", "DEVICE_ID", "CREATED_BY") //
			.columns("_id", "FOLDER_CLOUD_ID", "FOLDER_PATH", "FOLDER_NAME", "FORMAT", "CLOUD_TYPE", "PASSWORD", "PASSWORD_CRYPTO_MODE", "POSITION", "SHORTENING_THRESHOLD", "DEVICE_ID", "CREATED_BY") //
