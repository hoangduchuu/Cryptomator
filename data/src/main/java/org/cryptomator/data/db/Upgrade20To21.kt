...
@Singleton
internal class Upgrade20To21 @Inject constructor() : DatabaseUpgrade(20, 21) {

    override fun internalApplyTo(db: Database, origin: Int) {
        db.beginTransaction()
        try {
            addPolicyIdToVaultSchema(db)
    ...
    private fun addPolicyIdToVaultSchema(db: Database) {
        // Step 1: Rename old table
        Sql.alterTable("VAULT_ENTITY").renameTo("VAULT_ENTITY_OLD").executeOn(db)

        // Step 2: Create new table with POLICY_ID column
        ...
        db.execSQL("""
            INSERT INTO VAULT_ENTITY (
                ...
                ETAG, STATUS, FULL_LOCAL_PATH, SIZE, POLICY_ID
            )
            SELECT 
